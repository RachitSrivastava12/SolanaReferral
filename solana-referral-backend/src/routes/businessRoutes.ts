...
router.get('/campaign-stats', async (req: Request, res: Response) => {
    try {
        ...
        const referrals = await Referral.find({ campaign: campaignId }).populate('campaign');
        ...
        for (const ref of referrals.filter(r => r.paid)) {
            ...
            if (campaign) {
                // Each completion pays 2x the reward (referrer + referred)
                totalRewardsPaid += campaign.rewardperReferral * 2;
            }
        }
